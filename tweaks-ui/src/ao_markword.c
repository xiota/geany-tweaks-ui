...
#define AO_MARKWORD_GET_PRIVATE(obj) \
  (G_TYPE_INSTANCE_GET_PRIVATE((obj), AO_MARKWORD_TYPE, AoMarkWordPrivate))
...
static void ao_mark_word_class_init(AoMarkWordClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_ENABLE_MARKWORD,
      g_param_spec_boolean(
          "enable-markword", "enable-markword",
          "Whether to mark all occurrences of a word when double-clicking it",
          TRUE, G_PARAM_WRITABLE));

  g_object_class_install_property(
      g_object_class, PROP_ENABLE_MARKWORD_SINGLE_CLICK_DESELECT,
      g_param_spec_boolean(
          "enable-single-click-deselect", "enable-single-click-deselect",
          "Enable deselecting a previous highlight by single click", TRUE,
          G_PARAM_WRITABLE));
}
...
static void clear_marker(void) {
  /* There might be a small risk that the current document is not the one we
   * want but this is very unlikely. */
...
static gboolean on_editor_button_press_event(GtkWidget *widget,
  ...
  if (event->button == 1) {
    AoMarkWordPrivate *priv = AO_MARKWORD_GET_PRIVATE(bm);
    if (!priv->enable_markword) return FALSE;

    if (event->type == GDK_BUTTON_PRESS) {
      if (priv->enable_single_click_deselect) clear_marker();
    } else if (event->type == GDK_2BUTTON_PRESS) {
      if (priv->double_click_timer_id == 0)
        priv->double_click_timer_id =
            g_timeout_add(DOUBLE_CLICK_DELAY, mark_word, bm);
    }
  }
  return FALSE;
}
...
void ao_mark_editor_notify(AoMarkWord *mw, GeanyEditor *editor,
                           SCNotification *nt) {
  // If something is about to be deleted and there is selected text clear the
  // markers
...
static void connect_document_button_press_signal_handler(
    AoMarkWord *mw, GeanyDocument *document) {
  ...
  plugin_signal_connect(geany_plugin, G_OBJECT(document->editor->sci),
                        ...
                        G_CALLBACK(on_editor_button_press_event), mw);
}
...
void ao_mark_document_close(AoMarkWord *mw, GeanyDocument *document) {
  ...
  g_signal_handlers_disconnect_by_func(document->editor->sci,
                                       on_editor_button_press_event, mw);
}
...
AoMarkWord *ao_mark_word_new(gboolean enable, gboolean single_click_deselect) {
  return g_object_new(AO_MARKWORD_TYPE, "enable-markword", enable,
