...
#define AO_COLORTIP_GET_PRIVATE(obj) \
  (G_TYPE_INSTANCE_GET_PRIVATE((obj), AO_COLORTIP_TYPE, AoColorTipPrivate))

// This is helpful for making the color-tip larger on 4K screens or for people
// with less acute vision
...
static gboolean on_editor_button_press_event(GtkWidget *widget,
  ...
  if (event->button == 1) {
    if (event->type == GDK_2BUTTON_PRESS) {
      ...
      if (!priv->enable_double_click_color_chooser) return FALSE;
...
void ao_color_tip_editor_notify(AoColorTip *colortip, GeanyEditor *editor,
                                SCNotification *nt) {
...
static void connect_document_button_press_signal_handler(
    AoColorTip *colortip, GeanyDocument *document) {
  g_return_if_fail(DOC_VALID(document));

  plugin_signal_connect(geany_plugin, G_OBJECT(document->editor->sci),
                        "button-press-event", FALSE,
                        G_CALLBACK(on_editor_button_press_event), colortip);
}

static void connect_documents_button_press_signal_handler(
    AoColorTip *colortip) {
  guint i = 0;
...
static void ao_color_tip_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    ...
    case PROP_ENABLE_DOUBLE_CLICK_COLOR_CHOOSER:
      ...
      /* If the plugin is loaded while Geany is already running, we need to
       * connect the button press signal for open documents, if Geany is just
       * booting, it happens automatically */
...
void ao_color_tip_document_close(AoColorTip *colortip,
                                 GeanyDocument *document) {
  g_return_if_fail(DOC_VALID(document));

  g_signal_handlers_disconnect_by_func(document->editor->sci,
                                       on_editor_button_press_event, colortip);
}
...
static void ao_color_tip_class_init(AoColorTipClass *klass) {
  ...
  g_object_class_install_property(
      g_object_class, PROP_ENABLE_COLORTIP,
      g_param_spec_boolean(
          "enable-colortip", "enable-colortip",
          "Whether to show a calltip when hovering over a color value", TRUE,
          G_PARAM_WRITABLE));

  g_object_class_install_property(
      g_object_class, PROP_ENABLE_DOUBLE_CLICK_COLOR_CHOOSER,
      g_param_spec_boolean("enable-double-click-color-chooser",
                           "enable-double-click-color-chooser",
                           "Enable starting the Color Chooser when double "
                           "clicking on a color value",
...
AoColorTip *ao_color_tip_new(gboolean enable_tip,
  ...
  return g_object_new(AO_COLORTIP_TYPE, "enable-colortip", enable_tip,
                      "enable-double-click-color-chooser",
